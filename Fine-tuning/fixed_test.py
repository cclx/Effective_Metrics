...
def test(model, test_dataloader, epochs, device):
    ...
    for epoch_i in range(epochs):
        ...
        avg_cola_loss = total_cola_loss / len(test_dataloader)
        avg_prob_loss = total_prob_loss / len(test_dataloader)
        ...
        print("  Training epcoh took: {:}".format(testing_time))
        
        avg_test_matthews = total_eval_matthews / len(test_dataloader)
        print("  Matthews: {0:.5f}".format(avg_test_matthews))
        ...
        testing_stats.append(
            {
                ...
                'Test. Matthews.': avg_test_matthews,
...
if __name__ == '__main__':
    ...
    for j in range(1, 40, 4):
        for i in range(7, 8):
            model = torch.load('parameter8/bertClass-large_mean{seed}_save_{index}.pt'.format(seed = j, index = i))
            model.eval()
            model.to(device)
