...
from model import bertLinearClassification
from dataPreproce import True_dataPrepare
from utils import format_time, flat_accuracy, flat_matthews


# ---------------------------------------- Train ----------------------------------------------------
def train(model, optimizer, scheduler, train_dataloader, validation_dataloader,epochs, device, seed):
    ...
    for epoch_i in range(epochs):
        ...
        training_stats.append(
            {
                'epoch': epoch_i + 1,
                'Training Loss': "{0:.5f}".format(avg_train_loss),
    ...
    #torch.save(model, 'bertClass_save_notfixed_large.pt')
    #print("Model saved !")
...
if __name__ == '__main__':
    ...
    for j in range(1, 41, 4):
        ...
        train_stats = train(model, optimizer, scheduler, train_dataloader, validation_dataloader, epochs, device, j)
        model_stats.append(train_stats)

    
    with open("cola-seed1~40.csv", "w") as f:
        for j in range(len(model_stats)):
            for k in range(len(model_stats[j])):
                for index, key in enumerate(model_stats[j][k]):
                    f.write("%s\t%s\n" % (key, model_stats[j][k][key]))   
